...
interface ProductCategoryRecordDetailsDialogProps {
  visible: boolean;
  onHide: () => void;
  selectedRecord: any;
}

const ProductCategoryRecordDetailsDialog: React.FC<
  ProductCategoryRecordDetailsDialogProps
> = ({ visible, onHide, selectedRecord }) => {
  return (
    <Dialog
      header="Product Category Details"
      visible={visible}
      footer={
        <Button
          label="Close"
      ...
      <div className="flex flex-col items-center p-6">
        ...
        <div className="mt-4 w-full max-w-4xl space-y-6 text-sm">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
              <p>
                <strong>BC Category ID:</strong>{" "}
                {selectedRecord?.bc_category_id || "N/A"}
              </p>
              <p>
                <strong>BC Category Code:</strong>{" "}
                {selectedRecord?.bc_category_code || "N/A"}
              </p>
            </div>
          </div>
...
export default ProductCategoryRecordDetailsDialog;
