...
const DeleteRecordsDialog: React.FC<DeleteRecordsDialogProps> = ({
  ...
  const deleteMutation = useMutation({
    mutationFn: postToBulkDestroyProductSubcategories,
    onSuccess: () => {
      // queryClient.invalidateQueries({ queryKey: ["counties"] });
      queryClient.resetQueries({ queryKey: ["product-subcategories"] });
      primeReactToast.success("Product Sub Category deleted successfully");
      setSelectedItems([]);
      onHide();
    },
  });
    ...
    >
      <div className="flex flex-col items-center p-3">
        ...
        <p className="text-center m-0">
          This action cannot be undone. All the selected rows will be
          permanently deleted.
        </p>
